...
#特定のCSVファイルを読込み、カスタマイズを可能とする。例えば、区切るタイミングを「。」「】」「改行」「>」を個人でカスタマイズ可能とする。
